...
Util::Util(const ML::Device& device, TensorDataType tensorDataType, TensorLayout tensorLayout) :
	m_device(device),
...
bool Util::CreateUpsampleLayer(const uint32_t inputSizes[4], uint64_t& inputBufferRequiredSize,
...
bool Util::CreateConvolutionLayer(const uint32_t inputSizes[4], const uint32_t* filterSizes, bool useBiasAndActivation,
...
bool Util::CreateAdditionLayer(const uint32_t inputSizes[4], Operator& opOut)
{
	// Describe input and output tensors
	uint32_t strides[4];
...
void Util::CreateWeightTensors(WeightMapType& weights, const char* convLayerName, const char* scaleLayerName,
